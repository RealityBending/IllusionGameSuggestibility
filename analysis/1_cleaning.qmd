...
plot_distribution(df[df$Participant %in% dfsub$Participant[1:10], ])
```
...
plot_distribution(df[df$Participant %in% dfsub$Participant[10:15], ])
```
...
outliers <- dfsub |> 
  ...
  standardize() |> 
  performance::check_outliers(method = c("mahalanobis"))
                              
table <- dfsub |> 
  select(Participant, IPIP6_SD, IPIP6_Duration, PID5_SD, PID5_Duration) |> 
  mutate(Participant = fct_reorder(Participant, desc(attributes(outliers)$data$Distance_Mahalanobis))) |> 
  arrange(Participant)
...
data.frame(Participant = c("Average"), t(sapply(table[2:ncol(table)], mean, na.rm = TRUE))) |>
  ...
  gt::data_color(columns = ends_with("Duration"), 
                 direction="column", palette = "viridis", reverse=TRUE) |> 
  gt::data_color(columns = ends_with("SD"), 
                 direction="column", palette = "viridis") |> 
