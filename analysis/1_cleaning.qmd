...
```{r}
outliers <- c("S044")
outliers_half <- c("S031", "S036", "S051")
```
...
plot_distribution(df[df$Participant %in% dfsub$Participant[1:40], ])
```
...
plot_distribution(df[df$Participant %in% dfsub$Participant[41:80], ])
```


:::

We discarded `r length(outliers_half)` participant's second blocks.
